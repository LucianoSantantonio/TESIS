# -*- coding: utf-8 -*-
"""
Created on Wed Oct 29 20:18:12 2025
...
# <-- AÑADE ESTE BUCLE EN LUGAR DE LA LÍNEA ANTERIOR
for barra in barras_container:
    altura = barra.get_height()
    
    # Formateamos el texto de la etiqueta (ej: "75.4%")
    etiqueta = f'{altura:.1f}%'
    
    # Colocamos el texto en la posición correcta
    plt.text(
        barra.get_x() + barra.get_width() / 2.0, # Posición X (centro de la barra)
        altura,                               # Posición Y (justo encima de la barra)
        etiqueta,                             # El texto a mostrar
        ha='center',                          # Alineación horizontal (centrado)
        va='bottom'                           # Alineación vertical (el texto empieza 'encima' de la altura)
    )

# <-- LÍNEA 2: AÑADIMOS LAS ETIQUETAS A CADA BARRA
# fmt='%.1f%%' significa: formato de punto flotante (f) con 1 decimal (.1)
# seguido del símbolo de porcentaje literal (%%)
plt.bar_label(barras_container, fmt='%.1f%%')
...
nombre_grafico_salida = 'grafico_aciertos_con_valores.png' # Cambié el nombre
